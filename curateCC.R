...
input_folder <- "/Users/lfrank/Desktop/PCH-dCas9_working_copy/test_images/dCas9-EGFP/2020-06-26_16-57-28_segmentCC_results/"
...
mean_sd_projection_seg <- mean_sd_projection[which(mean_sd_projection$mean_dapi_cc!=0),]
...
for (i in 1:length(rownames(positions))){
  ...
  if (flawless_image!="y"){
    ...
    for(k in 1:j){
      ...
      annotation <- c(annotation, readline('Evaluate segmentation for the highlighted cell as good(g) or bad(b)     '))
    }
  }
  else{
...
colnames(mean_sd_projection_curated)[25] <- "annotation"
